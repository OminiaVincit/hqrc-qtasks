...
CAPA=4
WIDTH=0.2
P=seed_1_mdeg_7_mvar_7_thres_0.0_delays_$DELAYS$TS

for alpha in 0.2
do
for bc in 1.0
do
python $EXE --nproc $NPROC --units $NSPINS --seed $SEED --dynamic $DYNAMIC --deg_delays $DELAYS --thres $THRES --tmin $TMIN --tmax $TMAX --ntaus $NTAUS --alpha $alpha --bcoef $bc --virtuals $V --length $T --max_deg $DEG --max_window $WD --max_num_var $VAR --savedir $SAVE
