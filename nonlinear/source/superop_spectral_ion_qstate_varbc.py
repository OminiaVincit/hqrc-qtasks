...
def dump_eigenval_job(savedir, basename, Nspins, Nenvs, alpha, tauB, bcls, J, s_prep, idx, send_end):
    """
    Dump eigenvalues of superoperator
    """
    print('Start pid={} with size {} (from {} to {})'.format(idx, len(bcls), bcls[0], bcls[-1]))
    results = dict()
    for bc in bcls:
        B = J/bc
    ...
    print('Finished pid={} with size {} (from {} to {}) Num eigs = {}'.format(idx, len(bcls), bcls[0], bcls[-1], len(egvals)))
...
if __name__  == '__main__':
    ...
    parser.add_argument('--bcmin', type=float, default=0, help='Minimum of bc')
    parser.add_argument('--bcmax', type=float, default=2.5, help='Maximum of bc')
    ...
    J, Nspins, Nenvs, alpha, tauB = args.max_energy, args.nspins, args.nenvs, args.alpha, args.tauB
    bcmin, bcmax, nbcs, nc, edidx, bgidx = args.bcmin, args.bcmax, args.nbcs, args.nc, args.edidx, args.bgidx
    basename = '{}_nspins_{}_nenvs_{}_a_{}_bcmin_{}_bcmax_{}_nbcs_{}_tauB_{}'.format(args.basename, Nspins, Nenvs, alpha, bcmin, bcmax, nbcs, tauB)
    ...
    if os.path.isfile(savedir) == False:
        ...
        logger.info('Nspins={},Nenvs={},alpha={},tauB={},bcmin={},bcmax={},nbcs={}'.format(Nspins, Nenvs, alpha, tauB, bcmin, bcmax, nbcs))
        ...
        nproc = min(len(bcls), args.nproc)
        lst = np.array_split(bcls, nproc)
        ...
        for ridx in range(bgidx, edidx):
            ...
            for pid in range(nproc):
                ...
                p = multiprocessing.Process(target=dump_eigenval_job, \
                    args=(bindir, basename, Nspins, Nenvs, alpha, tauB, bcs, J, s_prep, pid, send_end))
                jobs.append(p)
