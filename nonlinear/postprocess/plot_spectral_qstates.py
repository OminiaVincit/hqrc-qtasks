...
if __name__  == '__main__':
    ...
    parser.add_argument('--posfix', type=str, default='tmin_0.0_tmax_25.0_ntaus_250_eig_id_124')
    ...
    ax1.set_title('$|\lambda_1|/|\lambda_2|$', fontsize=24)
    if ptype == 0:
        ...
        ax1.set_xlim([lo, hi])
        ax1.set_xticks(list(range(lo, hi + 1)))
    ax1.set_xlabel('$\\tau B$', fontsize=24)
    #ax1.scatter3D(ild2[:, 0], ild2[:, 1], ild2[:, 2], cmap='Green', rasterized=True)

    # # Plot 1/|lambda_2|, |lambda_2| / |lambda_3|
    ax2 = plt.subplot2grid((2,1), (1,0), colspan=1, rowspan=1)
    ax2.set_title('$|\lambda_2| / |\lambda_3|$', fontsize=24)
    if ptype == 0:
        ...
        ax2.set_xlim([lo, hi])
        ax2.set_xticks(list(range(lo, hi + 1)))
    ax2.set_xlabel('$\\tau B$', fontsize=24)
    plt.tight_layout()
