...
def IPC_compute(qparams, ipcparams, length, ntrials, ranseed, log_filename, savedir, posfix, writedelay):
    logger = get_module_logger(__name__, log_filename)
    logger.info(log_filename)

    hqrc.get_IPC(qparams, ipcparams, length, logger=logger, ranseed=ranseed, Ntrials=ntrials, \
        savedir=savedir, posfix=posfix, writedelay=writedelay, type_input=1)
...
if __name__  == '__main__':
    ...
    basename = '{}_{}_mdelay_{}_mdeg_{}_mvar_{}_thres_{}_T_{}'.format(\
        basename, dynamic, max_delay, max_deg, max_num_var, thres, length)
    log_filename = os.path.join(logdir, '{}.log'.format(basename))
    logger = get_module_logger(__name__, log_filename)
    logger.info(log_filename)

    # Make IPCparams
    ipcparams = IPCParams(max_delay, max_deg, max_num_var, thres, chunk)
        ...
        for V in virtuals:
            ...
            for x in tx:
                ...
                p = multiprocessing.Process(target=IPC_compute, \
                    args=(qparams, ipcparams, length, ntrials, ranseed, log_filename, savedir, posfix, writedelay))
                jobs.append(p)
