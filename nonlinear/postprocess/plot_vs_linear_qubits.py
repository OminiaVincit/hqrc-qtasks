...
avg_qrc_ys['N_m=4,K=5'] = np.array([0.990502, 0.952178, 0.919598, 0.911393])
std_qrc_ys['N_m=4,K=5'] = np.array([0.000451, 0.001100, 0.000436, 0.000700])
...
avg_qrc_ys['N_m=4,K=15'] = np.array([0.990577, 0.963902, 0.935524, 0.916531])
std_qrc_ys['N_m=4,K=15'] = np.array([0.000460, 0.000714, 0.000475, 0.000565])
...
for ftype in ['png', 'svg']:
    ...
    for label in avg_qrc_ys.keys():
        ys = avg_qrc_ys[label]
        es = std_qrc_ys[label]
        ax.plot(xs, ys, label='${}$'.format(label), alpha = 0.8, marker='s', markeredgecolor='k', \
                            linewidth=3, markersize=10, c=putils.cycle[dcl])
        ax.fill_between(xs, ys - es, ys + es, facecolor=putils.cycle[dcl], alpha=0.2)
        dcl += 1
    #ax.set_yscale('log')
    ax.legend()
    ax.set_xlabel('$N_e$')
    ...
    plt.savefig('tau_10.0_a_1.0_bc_1.0_1000_3000_1000_fidelity.{}'.format(ftype), bbox_inches='tight', transparent=transparent, dpi=600)
